...
class SchedulerOutputProcessorMixin:
    ...
    def _resolve_spec_overlap_token_ids(
        ...
        # [Dynamic Steps] Set spec_steps from batch.spec_info if available
        if hasattr(batch, 'spec_info') and hasattr(batch.spec_info, 'spec_steps'):
            result.spec_steps = batch.spec_info.spec_steps
        else:
            result.spec_steps = self.draft_worker.speculative_num_steps
    ...
    def process_batch_result_decode(
        ...
        if not batch.spec_algorithm.is_none():
            self.update_spec_metrics(batch.batch_size(), result.num_accepted_tokens, result.spec_steps, result.avg_throughput)
