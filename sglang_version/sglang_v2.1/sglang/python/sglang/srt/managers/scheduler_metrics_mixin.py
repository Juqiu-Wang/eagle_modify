...
class SchedulerMetricsMixin:
    def init_metrics(
        ...
        # [Dynamic Steps] The sum of draft steps for the recent `decode_log_interval` batches
        self.spec_num_draft_steps = 0
        # [Dynamic Steps] The sum of avg_throughput values for averaging
        self.spec_avg_throughput_sum = 0.0
    ...
    def update_spec_metrics(self: Scheduler, bs: int, num_accepted_tokens: int, current_draft_steps: int = 0, avg_throughput: float = 0.0):
        self.spec_num_accepted_tokens += num_accepted_tokens + bs
        self.spec_num_forward_ct += bs
        self.num_generated_tokens += num_accepted_tokens
        # [Dynamic Steps] Accumulate draft steps for calculating average
        self.spec_num_draft_steps += current_draft_steps * bs
        # [Dynamic Steps] Accumulate avg_throughput for averaging
        self.spec_avg_throughput_sum += avg_throughput * bs
    ...
    def log_decode_stats(
        ...
        if self.spec_algorithm.is_none():
            ...
            spec_accept_rate = 0
            avg_draft_steps = 0
            draft_only_accept_len = 0
            avg_throughput = 0
        else:
            spec_accept_length = (
                self.spec_num_accepted_tokens / self.spec_num_forward_ct - 1
            )

             # [Dynamic Steps] Calculate average draft steps and draft-only accept length
            avg_draft_steps = (
                self.spec_num_draft_steps / self.spec_num_forward_ct
                if self.spec_num_forward_ct > 0
                else 0
            )
            
            # [Dynamic Steps] Calculate average throughput threshold
            avg_throughput = (
                self.spec_avg_throughput_sum / self.spec_num_forward_ct
                if self.spec_num_forward_ct > 0
                else 0
            )
            
            spec_accept_rate = (
                spec_accept_length / avg_draft_steps
                if avg_draft_steps > 0
                else 0
            )
            ...
            self.spec_num_accepted_tokens = 0
            self.spec_num_forward_ct = 0
            self.spec_num_draft_steps = 0  # [Dynamic Steps] Reset draft steps counter
            self.spec_avg_throughput_sum = 0.0  # [Dynamic Steps] Reset avg_throughput sum
            msg += f"accept len: {spec_accept_length:.2f}, avg_draft_steps: {avg_draft_steps:.2f}, accept rate: {spec_accept_rate:.2f}, avg_throughput: {avg_throughput:.3f}, "
        cache_hit_rate = 0.0
