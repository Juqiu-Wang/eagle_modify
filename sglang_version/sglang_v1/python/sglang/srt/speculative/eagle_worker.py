...
class EAGLEWorker(TpModelWorker):
    ...
    def __init__(
        ...
        # 动态步数优化：根据平均接受长度动态选择draft model的前向步数
        # 关键点：draft阶段使用动态步数，但verify阶段始终使用最大draft_token_num
        # 这样可以确保CUDA graph的input_ids尺寸固定，避免tensor大小不匹配错误
        self.avg_acc_len = float(self.speculative_num_steps)
        self.ema_alpha = 0.05  # 指数移动平均的衰减因子
        self.current_draft_steps = self.speculative_num_steps  # 当前选择的动态步数
    ...
    def forward_batch_generation(self, batch: ScheduleBatch) -> GenerationBatchResult:
        ...
        if batch.forward_mode.is_extend() or batch.is_extend_in_batch:
            ...
            # 计算实际使用的draft tokens数量
            # EAGLE3树结构: 
            # 第1层: topk个token
            # 第2到第n层: 每层topk²个token (每个token分topk个分支)
            # 总共: topk + topk² × (spec_steps - 1) per sequence
            # 对于batch: batch_size × [topk + topk² × (spec_steps - 1)]
            # spec_steps 是实际使用的动态步数，存储在spec_info中
            num_draft_tokens = batch.batch_size() * (self.topk + self.topk * self.topk * (spec_info.spec_steps - 1))
            
            return GenerationBatchResult(
                logits_output=logits_output,
                next_token_ids=verify_output.verified_id,
                num_accepted_tokens=sum(verify_output.accept_length_per_req_cpu),
                num_draft_tokens=num_draft_tokens,  # 新增
    ...
    def draft(self, batch: ScheduleBatch):
        ...
        # [新增] 动态步数选择逻辑
        if self.avg_acc_len < 0.8:
            current_step = 1
        elif self.avg_acc_len < 1.6:
            current_step = 2
        elif self.avg_acc_len < 3.2:
            current_step = 4
        else:
        ...
        if can_cuda_graph:
            ...
            # [修改] 裁剪 out_cache_loc 到当前步数需要的长度
            # out_cache_loc 原本是按 max_steps 分配的，需要裁剪为 current_step 长度
            num_seqs = forward_batch.batch_size
            expected_cache_len = num_seqs * self.topk * current_step
            if forward_batch.out_cache_loc.numel() > expected_cache_len:
                forward_batch.out_cache_loc = forward_batch.out_cache_loc[:expected_cache_len]
        ...
        # [修改] 始终使用最大的 draft_token_num 以匹配 target worker 的 CUDA graph capture 尺寸
        # 这样可以确保 verify 阶段的 input_ids 大小固定，不会导致 CUDA graph replay 失败
        (
            tree_mask,
            position,
            retrive_index,
            ...
            draft_tokens,
            batch.seq_lens,
            batch.seq_lens_sum,       # arg 6: seq_lens_sum
            self.topk,                # arg 7: topk
            current_step,             # arg 8: spec_steps [修改] 传入当前步数
            self.speculative_num_draft_tokens,  # arg 9: num_verify_tokens [修改] 始终使用最大值
        )
        ...
        return EagleVerifyInput(
            ...
            spec_steps=current_step,  # [保留] 仍记录当前实际使用的步数供统计
            topk=self.topk,
            draft_token_num=self.speculative_num_draft_tokens,  # [修改] 使用最大值
    ...
    def draft_forward(self, forward_batch: ForwardBatch):
        ...
        # [修改] 计算实际可用的候选token数量，并pad到最大尺寸
        # EAGLE树结构: step 0 生成 topk 个，后续每步生成 topk^2 个
        # 实际候选数 = topk + (current_step - 1) * topk^2
        actual_candidates = self.topk + (self.speculative_num_steps - 1) * (self.topk ** 2)
        actual_num_draft_tokens = min(actual_candidates + 1, self.speculative_num_draft_tokens)
        
        # 使用实际可用的数量调用 organize_draft_results
        parent_list, top_scores_index, draft_tokens = organize_draft_results(
            score_list, token_list, parents_list, actual_num_draft_tokens
        )
        
        # [新增] 如果实际token数少于配置的最大值，需要pad到最大尺寸以匹配CUDA graph
        if actual_num_draft_tokens < self.speculative_num_draft_tokens:
            bs = draft_tokens.shape[0]
            # draft_tokens shape: (bs, actual_num_draft_tokens - 1)
            # 需要pad到 (bs, self.speculative_num_draft_tokens - 1)
            pad_size = self.speculative_num_draft_tokens - actual_num_draft_tokens
            # 用第一个token重复pad（任意有效token都可以，因为在verify时这些位置不会被使用）
            pad_tokens = draft_tokens[:, :1].repeat(1, pad_size)
            draft_tokens = torch.cat([draft_tokens, pad_tokens], dim=1)
            
            # 同样pad top_scores_index
            # top_scores_index shape: (bs, actual_num_draft_tokens - 1)
            pad_indices = top_scores_index[:, :1].repeat(1, pad_size)
            top_scores_index = torch.cat([top_scores_index, pad_indices], dim=1)
    ...
    def verify(self, batch: ScheduleBatch, spec_info: EagleVerifyInput):
        ...
        # 注意：accept_length 表示接受的draft tokens数量（不包括最后强制采样的token）
        # 实际每轮生成的tokens数 = accept_length + 1
