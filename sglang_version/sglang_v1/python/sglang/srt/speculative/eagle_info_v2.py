...
class EagleVerifyInputV2Mixin:
    def prepare_for_v2_verify(
        ...
        # [新增] 如果会使用 CUDA graph，需要填充 input_ids 到固定长度
        if can_run_cuda_graph:
            graph_runner = target_worker.model_runner.graph_runner
            # 获取 CUDA graph 录制时的 draft_token_num
            expected_draft_token_num = graph_runner.num_tokens_per_bs
            current_draft_token_num = self.draft_token_num
            
            if current_draft_token_num < expected_draft_token_num:
                # 需要填充到固定长度
                padding_len = bs * (expected_draft_token_num - current_draft_token_num)
                if padding_len > 0:
                    padding = torch.zeros(
                        padding_len,
                    ...
                    batch.input_ids = torch.cat([batch.input_ids, padding])
                    
                    # 同时需要填充 out_cache_loc
                    padding_cache = torch.zeros(
                        padding_len,
                        dtype=batch.out_cache_loc.dtype,
                        device=batch.out_cache_loc.device
                    )
                    batch.out_cache_loc = torch.cat([batch.out_cache_loc, padding_cache])
