...
class EAGLEDraftCudaGraphRunner:
    ...
    def capture(self): # [修改]
        ...
        for step in self.supported_steps:
            ...
            self.output_buffers = {} # [修改] 每次必须重置，否则可能残留引用
            ...
            self.output_buffers_by_step[step] = self.output_buffers # [新增]

        # 恢复原始的最大步数，并将 graphs 和 output_buffers 指向最大步数的图 (作为默认 fallback)
        self.speculative_num_steps = original_steps
        self.graphs = self.graphs_by_step[original_steps]
        self.output_buffers = self.output_buffers_by_step[original_steps]
    ...
    # [修改] 确保函数签名包含 step 参数，并给予默认值
    def replay(self, forward_batch: ForwardBatch, step: int = None):
        ...
        if step is None:
            ...
            # 如果请求的步数没有对应的图，选择最接近且不小于的步数
            available_steps = sorted(self.graphs_by_step.keys())
            # 找到大于等于 step 的最小值
            larger_steps = [s for s in available_steps if s >= step]
            if larger_steps:
                step = min(larger_steps)
            else:
                # 如果没有更大的，使用最大的
