...
class Scheduler(
    ...
    def get_internal_state(self, recv_req: GetInternalStateReq):
        ...
        if not self.spec_algorithm.is_none() and self.spec_total_num_forward_ct > 0:
            ...
            # 添加总体接收率：总接受token数 / (总forward_ct × avg_draft_steps)
            # 注意：接收率 = avg_accept_length / avg_draft_steps
            avg_accept_length = self.spec_total_num_accepted_tokens / self.spec_total_num_forward_ct
            # 使用配置的步数作为平均步数（如果有动态调整，这里会略有偏差）
            avg_draft_steps = self.server_args.speculative_num_steps or 1
            ret["avg_spec_accept_rate"] = avg_accept_length / avg_draft_steps if avg_draft_steps > 0 else 0
        if RECORD_STEP_TIME:
    ...
    def set_internal_state(self, recv_req: SetInternalStateReq):
        ...
        if if_success:
            if not self.spec_algorithm.is_none() and self.spec_total_num_forward_ct > 0:
                ...
                # 输出总体接收率 = avg_accept_length / avg_draft_steps
                avg_draft_steps = self.server_args.speculative_num_steps or 1
                avg_spec_accept_rate = avg_spec_accept_length / avg_draft_steps if avg_draft_steps > 0 else 0
                logger.info(f"{avg_spec_accept_rate=}")
                logger.info(f"Total accepted tokens: {self.spec_total_num_accepted_tokens}, Total draft tokens used: {self.spec_total_num_draft_tokens}")
            self.spec_total_num_accepted_tokens = self.spec_total_num_forward_ct = self.spec_total_num_draft_tokens = 0
            for k, v in server_args_dict.items():
