...
class SchedulerMetricsMixin:
    def init_metrics(
        ...
        self.spec_num_draft_tokens = 0  # 新增：跟踪实际使用的draft tokens数量
        # The total number of accepted tokens and forward ct for the whole server lifetime
        self.spec_total_num_accepted_tokens = 0
        self.spec_total_num_forward_ct = 0
        self.spec_total_num_draft_tokens = 0  # 新增：总的draft tokens数量
    ...
    def update_spec_metrics(self: Scheduler, bs: int, num_accepted_tokens: int, num_draft_tokens: int = None):
        self.spec_num_accepted_tokens += num_accepted_tokens + bs
        self.spec_num_forward_ct += bs
        self.num_generated_tokens += num_accepted_tokens
        # 新增：记录实际使用的draft tokens数量
        if num_draft_tokens is not None:
            self.spec_num_draft_tokens += num_draft_tokens
    ...
    def log_decode_stats(
        ...
        if self.spec_algorithm.is_none():
            ...
            # Calculate acceptance rate: accepted tokens / draft_step
            # 注意：接收率定义为平均每个draft step接受的token数
            # 这里使用实际的draft_step（而不是draft_tokens数量）
            # 获取当前的draft_steps
            current_draft_steps_for_rate = 0
            if self.spec_algorithm.is_eagle() and hasattr(self, 'draft_worker'):
                try:
                    if hasattr(self.draft_worker, 'current_draft_steps'):
                        current_draft_steps_for_rate = self.draft_worker.current_draft_steps
                except:
                    pass
            if current_draft_steps_for_rate == 0:
                current_draft_steps_for_rate = self.server_args.speculative_num_steps or 1
            
            spec_accept_rate = (
                spec_accept_length / current_draft_steps_for_rate
                if current_draft_steps_for_rate > 0
                else 0
            )
            
            # 获取EAGLE worker的动态步数信息
            current_avg_accept_len = 0.0
            current_draft_steps = 0
            if self.spec_algorithm.is_eagle() and hasattr(self, 'draft_worker'):
                try:
                    if hasattr(self.draft_worker, 'avg_acc_len'):
                        current_avg_accept_len = self.draft_worker.avg_acc_len
                    # 获取当前选择的动态步数（而不是配置的最大值）
                    if hasattr(self.draft_worker, 'current_draft_steps'):
                        current_draft_steps = self.draft_worker.current_draft_steps
                except:
                    pass
            ...
            self.spec_num_accepted_tokens = self.spec_num_forward_ct = self.spec_num_draft_tokens = 0
            
            # 显示accept length、accept rate、当前平均接受长度和当前选择的draft步数
            msg += f"accept len: {spec_accept_length:.2f}, accept rate: {spec_accept_rate:.2f}, "
            # 总是显示动态步数信息（即使为0也显示）
            msg += f"avg_acc_len: {current_avg_accept_len:.2f}, draft_steps: {current_draft_steps}, "
        cache_hit_rate = 0.0
