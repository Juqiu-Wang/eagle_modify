...
async def build_conversation(
    ...
    return req_obj, end_timestamp - start_timestamp
...
def main():
    ...
    bench_conversations = {}
    print(f'benchmark_list: {benchmark_list}')
    for bench_name, num_prompts in benchmark_list:
        ...
        elif bench_name == "frontier_reporter":
            bench_conversations.update(get_frontier_reporter_conversations(num_prompts))
        else:
    ...
    if args.skip_launch_server:
        ...
        for bench_name, conversation_list in bench_conversations.items():
            ...
            # 10 <= test prompt num  <= batch_size
            test_lens = max(10, min(batch_size, len(conversation_list)))
            test_conversation_list = conversation_list[:test_lens]
            # start_timestamp = time.perf_counter()
            outputs = asyncio.run(
                run_benchmark(test_conversation_list, server_args, client, semaphore)
            )
            cost_time = 0.0
            req_objs = []
            for req_obj, duration in outputs:
                cost_time += duration
                req_objs.append(req_obj)
                
            # duration = time.perf_counter() - start_timestamp
            completion_tokens = sum(req_obj.output_tokens for req_obj in req_objs)
            acc_length = send_get_accept_length_request(base_url)
            send_flush_cache_request(base_url)
            throughput = completion_tokens / cost_time
            print(
                f"bs=8:{bench_name=} {completion_tokens=} {acc_length=} {cost_time=}s {throughput=} token/s"
            )
        exit()
    ...
    for batch_size, steps, topk, num_draft_tokens in configs:
        ...
        for bench_name, conversation_list in bench_conversations.items():
            ...
            total_duration = time.perf_counter() - start_timestamp
            cost_time = 0.0
            req_objs = []
            for req_obj, duration in outputs:
                cost_time += duration
                req_objs.append(req_obj)
            avg_duration = cost_time / len(req_objs)
            completion_tokens = sum(req_obj.output_tokens for req_obj in req_objs)
            time.sleep(3)
            if steps > 0:
                acc_length = send_get_accept_length_request(base_url)
